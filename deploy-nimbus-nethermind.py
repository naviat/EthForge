...
valid_networks = ['HOLESKY', 'SEPOLIA']
...
parser = argparse.ArgumentParser(description='Validator Install Options',formatter_class=argparse.ArgumentDefaultsHelpFormatter)
parser.add_argument("--network", type=str, help="Sets the Ethereum network", choices=valid_networks, default="HOLESKY")
...
index = SelectionMenu.get_selection(valid_networks,title='Validator Install Quickstart',subtitle='Installs Nethermind EL / Nimbus BN / Nimbus VC / MEVboost\nSelect Ethereum network:')
...
if eth_network == "sepolia":
    ...
    answer = PromptUtils(Screen()).prompt_for_yes_or_no(
        "Install a Validator? If no, this will install a node only."
    )
    if not answer:
        NODE_ONLY=True
        MEVBOOST_ENABLED=False
...
if answer := PromptUtils(Screen()).prompt_for_yes_or_no(
